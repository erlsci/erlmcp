...
| M5b-7 | All examples on the new core. | CT suites green; docs link to them. | serious | dev plan M5b DoD | done | `db03e58`; Calculator (M2a), weather (M2b), client (M3a), sampling (M3b), task (M6a) — all CT suites pass. No example uses a deleted module. | **Amendment (examples rehabilitation):** M5b-7 was satisfied only for the `test/` CT suites. The user-facing `examples/` tree still referenced deleted 0.5 modules and could not compile. Extended by `task/0.6.0-examples`: all `examples/` files rewritten onto the 0.6.0 core, smoke CT suites added, examples folded into CI via `extra_src_dirs` in the `test` profile + explicit profile compilation in `ci.yml`. |
