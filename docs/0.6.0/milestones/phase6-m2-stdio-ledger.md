...
| P6M2-7 | Round-trip subprocess test: a **config-driven** server launched via `run.sh`, fed `initialize`+`initialized`+`tools/list` on stdin, returns the full catalog on stdout. | `test/scripts/test_stdio_roundtrip.sh` passes in CI against a config-driven server (a minimal test fixture or a config-driven `simple`; full example rehab stays P6-M5). | serious | handoff §1.8 | open | | The in-VM `simulate_input` path bypasses real I/O and hid bugs before — this exercises the real `user`-device path. |
