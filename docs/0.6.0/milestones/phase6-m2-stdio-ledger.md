...
| P6M2-9 | stdin **EOF triggers clean shutdown** of the subtree (the OS process exits; no hang). | CT/script: closing stdin terminates the transport and tears the subtree down cleanly (no `reader_died` error spew, exit code sane). | serious | handoff §3 (open: "server doesn't exit on stdin EOF") | done | This commit; CT: `erlmcp_stdio_lifecycle_SUITE:eof_shuts_down_transport`; script: `test_stdio_roundtrip.sh` EOF→node-halt assertion | Full chain: reader EOF → transport `{stop, normal}` → permanent child death triggers `one_for_all` cascade → intensity 0 exceeded → supervisor terminates → permanent app terminates → node halts. Verified end-to-end via subprocess test (node exits within timeout). |
...
| P6M2-16 | The `simple` server boots via `erl -noshell` with `rebar3 path --ebin` for code paths and `application:ensure_all_started/2` with `permanent` as the startup eval. No relx release; no `foreground`/`console` wrapper. Stdout carries protocol bytes only. | `grep -q "ensure_all_started.*permanent" examples/simple/run.sh`; `grep -q "rebar3.*path.*--ebin" examples/simple/run.sh`; `grep -q "\-noshell" examples/simple/run.sh`; round-trip (P6M2-7) green incl. first-line-is-JSON assertion. | serious | Phase 6 release route; handoff §1.1; the `foreground` echo/`-noinput` finding | done | db041be, this commit; `run.sh` uses `ensure_all_started(<app>, permanent)` + `-noshell` | **Amended:** relx dropped; `/2 permanent` ensures node halts on app death. The `foreground` wrapper is disqualified (echoes to stdout, disables stdin). The `rebar3 path` approach is simpler, correct, and doesn't require release infrastructure. |
