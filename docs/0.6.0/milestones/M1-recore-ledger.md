...
| M1-16 | The M1 spine modules are removed from `cover_excl_mods` and the gate holds ≥90% over the included set. | `cover_excl_mods` no longer lists the spine modules (`erlmcp_codec`, `erlmcp_model`, `erlmcp_capabilities`, `erlmcp_ctx`, `erlmcp_server_session`, `erlmcp_client_session`, `erlmcp_json_rpc`); CI `cover --min_coverage=90` passes. **Amendment (iteration 2):** original Verify named sups + registry which M1 does not re-seat (M2 work); corrected to name exactly the modules M1 implemented. Sups (`erlmcp_app`, `erlmcp_sup`, `erlmcp_server_sup`, `erlmcp_session_sup`, `erlmcp_transport_sup`) and `erlmcp_registry` remain excluded with M2 re-entry. | serious | M0 carry-forward #4; gives the coverage gate teeth | done | `9ed406d`; `cover_excl_mods` does not list any spine module; `rebar3 as test cover -v --min_coverage=90` passes at 94%. | `erlmcp`(M2a), `erlmcp_client`(M3), `erlmcp_transport_tcp`/`_stdio`(M4) stay excluded until their milestones. Skeletons leave as implemented. |
...
| M1-18 | A PropEr model of the JSON-RPC envelope and the session lifecycle passes. | `rebar3 proper -c`: all properties in `prop_envelope` (`prop_request_roundtrip`, `prop_response_roundtrip`, `prop_error_response_roundtrip`, `prop_notification_roundtrip`, `prop_codec_roundtrip`, `prop_batch_roundtrip`, `prop_malformed_batch_degrades`) and `prop_session` (`prop_session_lifecycle`) pass. **Amendment (iteration 2):** original Verify named `prop_envelope_roundtrip` which does not exist; corrected to the actual property names. | serious | dev plan M1 DoD; Phase 2 §9 | done | `9566a94` + `9ed406d`; `rebar3 proper -c` → 8/8 properties passed. | |
