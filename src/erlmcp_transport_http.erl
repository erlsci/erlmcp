...
-type state() ::
    #{url := string(),
      ...
      pending_requests := #{reference() => {binary(), non_neg_integer()}},
...
-spec start_link(http_opts()) -> gen_server:start_ret().
...
handle_call({send, Data}, _From, State) ->
    NewState = enqueue_request(Data, State),
    {reply, ok, process_request_queue(NewState)};
...
terminate(_Reason, #{pending_requests := Pending}) ->
    ...
    maps:foreach(fun(RequestId, _) -> httpc:cancel_request(RequestId) end, Pending),
    ok.
...
normalize_headers(Headers) ->
    ...
    UserHeaders = lists:map(fun({K, V}) -> {to_string(K), to_string(V)} end, Headers),
...
build_http_options(Opts) ->
    ...
    ++ maps:get(http_options, Opts, []).

-spec build_request_options(http_opts()) -> [term()].
build_request_options(_Opts) ->
    [{sync, false}, {body_format, binary}, {full_result, true}].

%%====================================================================
%% Internal functions - Request Handling
%%====================================================================

-spec enqueue_request(binary(), state()) -> state().
enqueue_request(Data, State) ->
    NewQueue = queue:in(Data, maps:get(message_queue, State)),
    State#{message_queue := NewQueue}.

-spec process_request_queue(state()) -> state().
process_request_queue(#{active_requests := Active, pool_size := PoolSize} = State)
...
process_request_queue(#{message_queue := Queue} = State) ->
    case queue:out(Queue) of
        {{value, Data}, NewQueue} ->
            ...
            process_request_queue(dispatch_request(Data, NewState));
        {empty, _} ->
            State
    end.

-spec dispatch_request(binary(), state()) -> state().
dispatch_request(Data, State) ->
    case perform_request(Data, State) of
        {ok, RequestId} ->
            NewPending = maps:put(RequestId, {Data, 0}, maps:get(pending_requests, State)),
                ...
                case process_response(Result) of
                    ...
                    {error, Reason} ->
                        case should_retry(Reason, Attempts, State) of
                            true ->
                                schedule_retry(RequestId, Data, Attempts + 1, NewState);
                            false ->
                                logger:error("HTTP request failed after ~p attempts: ~p",
                                             [Attempts + 1, Reason]),
                                NewState
                        end
                end,
...
process_response({{_Version, StatusCode, _ReasonPhrase}, Headers, Body})
    when StatusCode >= 200, StatusCode < 300 ->
    process_body(Body, Headers);
process_response({{_Version, StatusCode, ReasonPhrase}, _Headers, _Body}) ->
...
-spec schedule_retry(reference(), binary(), non_neg_integer(), state()) -> state().
schedule_retry(RequestId, Data, Attempts, State) ->
    ...
    NewPending = maps:put(RequestId, {Data, Attempts}, maps:get(pending_requests, State)),
    State#{pending_requests := NewPending}.
    ...
    case maps:get(RequestId, maps:get(pending_requests, State), undefined) of
        {Data, _} ->
            ...
            NewState =
                State#{pending_requests := NewPending,
                       active_requests := max(0, maps:get(active_requests, State) - 1)},
            dispatch_request(Data, NewState);
        _ ->
            State
    end.
